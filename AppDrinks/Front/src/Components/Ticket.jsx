...
function Ticket({ selectedDrinks, conteo, total, eliminarTrago }) {
    ...
    <>
      <div className="bg-white h-[70%] rounded-[50px] p-3 m-4 flex flex-col justify-start max-w-[500px] ">
        ...
        <div className="text-xl flex flex-cols-7 justify-around h-[80%]">
          <ul className="flex flex-col justify-start p-5 items-start col-span-3">
            {selectedDrinks.map((selectedDrink, index) => (
              <>
                <div className="flex flex-row gap-2">
                  <button
                    className="text-white text-sm bg-red-500 px-2 h-[25px] rounded-xl"
                    onClick={() => eliminarTrago(selectedDrink)}
                  >
                    x
                  </button>
                  <li key={index}>{selectedDrink.nombre}</li>
                </div>
              </>
            ))}
          </ul>
      ...
      <div className="my-4 flex justify-around">
        <button className="font-fredericka rounded-lg w-[40%] bg-red-500 px-2 hover:bg-red-600  font-bold text-[20px] hover:scale-105 transition">
          Borrar todo
        </button>
